...
Fix: correct seed setting inside Pipeline (use **clf__random_state**), plus
extra guards so it never crashes if a param isn't available.

Usage
  python -u src/survival_stack_fixed_lgbm_xgb_v2.py --data-root data \
    --out submissions/submission_survival.csv --mode quick --seeds 2025,1337
  # Stronger
  python -u src/survival_stack_fixed_lgbm_xgb_v2.py --data-root data \
    --out submissions/submission_survival_max.csv --mode max --seeds 2025,1337,4242

Notes
- LGBM/XGB are trained as censored-aware regressors on y_pseudo = -min(T, tau)
  with downweight 0.6 for censored (E=0), scored by IPCW-C@tau.
- Survival-native models: Coxnet, RSF, GBSA.
"""
import argparse
import os
import inspect
...
VERSION = "v2.3-lgbm-xgb-seedfix-2025-08-31"
...
def _safe_set_seed(pipe: Pipeline, seed: int):
    """Try common seed param names inside a Pipeline without crashing."""
    tried = [("clf__random_state", seed), ("clf__seed", seed), ("random_state", seed), ("seed", seed)]
    for k, v in tried:
        try:
            pipe.set_params(**{k: v})
            return
