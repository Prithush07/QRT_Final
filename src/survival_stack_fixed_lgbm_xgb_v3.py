#!/usr/bin/env python3
"""
Survival-native trainer (official IPCW-C @ 7y) + LightGBM/XGBoost boosters.
Fixes:
- Safe Coxnet alpha escalation (no ambiguous truth on numpy arrays).
- Robust seeding inside Pipelines.
- CENTER-grouped CV, stacking, multi-seed rank blend.

Run (quick):
  python -u src/survival_stack_fixed_lgbm_xgb_v3.py --data-root data --out submissions/submission_quick.csv \
    --mode quick --seeds 2025,1337 --top-genes 800 --top-effects 40 --tau 7
Run (max):
  python -u src/survival_stack_fixed_lgbm_xgb_v3.py --data-root data --out submissions/submission_max.csv \
    --mode max --seeds 2025,1337,4242 --top-genes 1000 --top-effects 48 --tau 7
"""
import argparse
import os
import inspect
...
VERSION = "v2.4-lgbm-xgb-alpha-fix-2025-08-31"
...
def _make_ohe():
    p = inspect.signature(OneHotEncoder).parameters
    if "sparse_output" in p:
        return OneHotEncoder(handle_unknown="ignore", sparse_output=False)
    return OneHotEncoder(handle_unknown="ignore", sparse=False)


def _rank_avg(arrs: List[np.ndarray]) -> np.ndarray:
    ranks = [pd.Series(a).rank(method="average").to_numpy() for a in arrs]
    return np.mean(ranks, axis=0)


def _safe_set_seed(pipe: Pipeline, seed: int):
    for k in ("clf__random_state", "clf__seed", "random_state", "seed"):
        try:
            pipe.set_params(**{k: seed})
            return
        except Exception:
            continue
...
def build_preprocessor(df: pd.DataFrame) -> ColumnTransformer:
    ...
    return ColumnTransformer([
        ...
        ("cat", Pipeline([("imp", SimpleImputer(strategy="constant", fill_value="MISSING")), ("oh", _make_ohe())]), cat_cols),
    ], remainder="drop", verbose_feature_names_out=False)

# -------------------- Grid runners --------------------

def _fit_with_alpha_escalation(model: Pipeline, X_tr, y_tr, is_coxnet: bool):
    if not is_coxnet:
        model.fit(X_tr, y_tr)
        return model
    tries = 0
    while True:
        try:
            ...
            # raise alphas x10 each try (up to 1e3)
            alphas = model.get_params().get("clf__alphas", None)
            if alphas is None:
                alphas = 10 ** np.linspace(-2, 1, 24)
            alphas = np.asarray(alphas, dtype=float)
            alphas = np.clip(alphas * 10.0, 1e-3, 1e3)
            model.set_params(clf__alphas=np.unique(alphas))
            tries += 1
            print(f"[WARN] Coxnet numerical -> raise alphas (try {tries}))")
            if tries >= 3:
                # last attempt; if it still fails, let it bubble
                model.fit(X_tr, y_tr)
                return model
...
def grid_run_surv(name: str, pipe: Pipeline, grid: Dict[str, List], X: pd.DataFrame, y_surv: np.ndarray, groups: pd.Series, tau: float, Xte: pd.DataFrame, folds: int) -> Tuple[str, Dict, np.ndarray, np.ndarray, List[float]]:
    ...
    for combo in product(*values):
        ...
        is_coxnet = any(isinstance(s, CoxnetSurvivalAnalysis) for _, s in model.steps)
        if is_coxnet:
            # set a stable default alpha grid once
            model.set_params(clf__alphas=10 ** np.linspace(-2, 1, 24), clf__max_iter=200000)
        ...
        for tr, va in gkf.split(X, groups=groups):
            ...
            _fit_with_alpha_escalation(model, X_tr, y_tr, is_coxnet)
...
def grid_run_reg(name: str, pipe: Pipeline, grid: Dict[str, List], X: pd.DataFrame, y_surv: np.ndarray, groups: pd.Series, tau: float, Xte: pd.DataFrame, folds: int) -> Tuple[str, Dict, np.ndarray, np.ndarray, List[float]]:
    ...
    for combo in product(*values):
        ...
        for tr, va in gkf.split(X, groups=groups):
            ...
            w = np.where(E == 1, 1.0, 0.6)
            try:
                model.fit(X_tr, y_pseudo, **{"clf__sample_weight": w})
            except TypeError:
...
def main():
    ...
    if int((~mask).sum()):
        print(f"[CLEAN] Dropped {(~mask).sum()} rows with missing/invalid OS labels.")
    ...
    specs = [("coxnet", cox, cox_grid, grid_run_surv), ("rsf", rsf, rsf_grid, grid_run_surv), ("gbsa", gbs, gbs_grid, grid_run_surv)]

    if _HAS_LGBM:
        lgbm = Pipeline([("pre", pre), ("clf", lgb.LGBMRegressor(objective="regression", subsample=0.8, colsample_bytree=0.8, n_jobs=-1, random_state=RNG))])
    ...
    for seed in seed_list:
        _safe_set_seed(rsf, seed)
        _safe_set_seed(gbs, seed)
        if _HAS_LGBM:
            _safe_set_seed(lgbm, seed)
        if _HAS_XGB:
            _safe_set_seed(xgbr, seed)
        ...
        oof_rank = _rank_avg([oof[k] for k in oof] + [oof_st])
        te_rank  = _rank_avg([te[k] for k in te] + [stk.predict(Zte)])
        oof_seeds.append(oof_rank)
        te_seeds.append(te_rank)

    oof_final = _rank_avg(oof_seeds) if len(oof_seeds) > 1 else oof_seeds[0]
    te_final  = _rank_avg(te_seeds)  if len(te_seeds) > 1 else te_seeds[0]
    ...
    print(f"[DONE] {args.out} rows={len(sub)} | VERSION={VERSION}")
